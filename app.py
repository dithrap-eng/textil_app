...
# -------------------------------
# RESUMEN DE CORTES (VERSIÓN FINAL)
# -------------------------------
...
if not df_cortes.empty:
    ...
    df_cortes["Consumo por prenda (m)"] = pd.to_numeric(df_cortes["Consumo por prenda (m)"], errors="coerce")
    
    # Calcular consumo por prenda si no existe o hay valores faltantes
    if df_cortes["Consumo por prenda (m)"].isna().any():
        df_cortes["Consumo por prenda (m)"] = df_cortes["Consumo total (m)"] / df_cortes["Prendas"]
    
    # Formatear para mostrar
    df_mostrar_cortes = df_cortes.copy()
    
    # Formatear números en estilo argentino
    df_mostrar_cortes["Consumo total (m)"] = df_mostrar_cortes["Consumo total (m)"].apply(
        lambda x: f"{x:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if pd.notna(x) else ""
    )
    
    df_mostrar_cortes["Consumo por prenda (m)"] = df_mostrar_cortes["Consumo por prenda (m)"].apply(
        lambda x: f"{x:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") if pd.notna(x) else ""
    )
    
    df_mostrar_cortes["Prendas"] = df_mostrar_cortes["Prendas"].apply(
        lambda x: f"{x:,.0f}".replace(",", ".") if pd.notna(x) else ""
    )
    
    # Mostrar solo las columnas que quieres ver
    columnas_a_mostrar = ["Fecha", "Nro Corte", "Artículo", "Tipo de tela", 
                          "Consumo total (m)", "Prendas", "Consumo por prenda (m)"]
    
    # Filtrar solo las columnas que existen
    columnas_existentes = [col for col in columnas_a_mostrar if col in df_mostrar_cortes.columns]
